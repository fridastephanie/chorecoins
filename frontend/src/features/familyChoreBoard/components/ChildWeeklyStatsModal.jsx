...
export default function ChildWeeklyStatsModal({ child, stats, onClose }) {
  ...
  return (
    <Modal title={`${child.firstName} Weekly Stats`} onClose={onClose}>
      {hasStats ? (
        <ul className="child-stats-list">
          {stats.map((week) => (
            <li key={week.id} className="child-stats-item">
              ...
              <div>
                <strong>Completed Chores:</strong> {week.completedChoresCount}
              </div>
              <div>
                <strong>Earned Coins:</strong> {week.earnedCoins}
              </div>
              <hr />
            </li>
          ))}
